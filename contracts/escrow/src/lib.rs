...
pub struct EscrowInit {
    ...
    pub refund_signers: Vec<Address>,
...
pub struct ReleaseProposal {
    pub nonce: u64,
    pub dists: Vec<(Address, i128)>,
}

fn read_u32(env: &Env, key: &DataKey) -> u32 {
    env.storage().persistent().get::<_, u32>(key).unwrap()
}
fn write_u32(env: &Env, key: &DataKey, v: u32) {
    env.storage().persistent().set(key, &v)
}
fn read_addr(env: &Env, key: &DataKey) -> Address {
    env.storage().persistent().get::<_, Address>(key).unwrap()
}
fn write_addr(env: &Env, key: &DataKey, v: &Address) {
    env.storage().persistent().set(key, v)
}
fn read_vec_addr(env: &Env, key: &DataKey) -> Vec<Address> {
    env.storage()
        .persistent()
        .get::<_, Vec<Address>>(key)
        .unwrap_or_else(|| Vec::new(env))
}
fn write_vec_addr(env: &Env, key: &DataKey, v: &Vec<Address>) {
    env.storage().persistent().set(key, v)
}
...
fn token_client<'a>(env: &'a Env, addr: &Address) -> token::Client<'a> {
    token::Client::new(env, addr)
}

fn is_member(list: &Vec<Address>, who: &Address) -> bool {
    list.iter().any(|a| a == who.clone())
}

fn push_unique(list: &mut Vec<Address>, who: &Address) {
    if !is_member(list, who) {
        list.push_back(who.clone());
    }
}

fn sum_amounts(dists: &Vec<(Address, i128)>) -> i128 {
    let mut s: i128 = 0;
    for (_, a) in dists.iter() {
        s += a;
    }
    s
}

fn ensure_payees_valid(payees: &Vec<Address>, dists: &Vec<(Address, i128)>) -> bool {
    for (p, _) in dists.iter() {
        if !is_member(payees, &p) {
            return false;
        }
    }
    true
...
impl MarketXEscrow {
    ...
    pub fn init(
        ...
        if emergency_threshold == 0 || emergency_threshold > emergency_admins.len() {
            panic!("bad-emergency-threshold");
        }
    ...
    pub fn set_emergency(env: Env, admin: Address, admins: Vec<Address>, threshold: u32) {
        let a = read_addr(&env, &DataKey::Admin);
        if admin != a {
            panic!("not-admin");
        }
    ...
    pub fn create_escrow(env: Env, id: u128, params: EscrowInit) {
        ...
        if params.release_threshold == 0 || params.release_threshold > params.release_signers.len()
        {
            panic!("bad-release-thresh");
        }
        if params.refund_threshold == 0 || params.refund_threshold > params.refund_signers.len() {
            panic!("bad-refund-thresh");
        }
        if params.arbiter_threshold == 0 || params.arbiter_threshold > params.arbiters.len() {
            panic!("bad-arb-thresh");
        }
        if params.payers.is_empty() || params.payees.is_empty() {
            panic!("empty-parties");
        }
        let e = Escrow {
            token: params.token,
    ...
    pub fn open_dispute(env: Env, id: u128, actor: Address) {
        ...
        if !(is_member(&e.payers, &actor) || is_member(&e.payees, &actor)) {
            panic!("no-right");
        }
        actor.require_auth();
        e.disputed = true;
    ...
    pub fn propose_release(env: Env, id: u128, signer: Address, dists: Vec<(Address, i128)>) {
        let mut e = read_escrow(&env, id);
        if e.closed {
            panic!("closed");
        }
        ...
        if !is_member(&e.release_signers, &signer) {
            panic!("not-release-signer");
        }
        signer.require_auth();
        if !ensure_payees_valid(&e.payees, &dists) {
            panic!("bad-payee");
        }
        let total = sum_amounts(&dists);
        if total <= 0 || total > e.balance {
            panic!("bad-total");
        }
        e.nonce += 1;
        let prop = ReleaseProposal {
            nonce: e.nonce,
            dists: dists.clone(),
        };
        env.storage()
            .persistent()
            .set(&DataKey::PendingRelease(id), &prop);
        let mut approvers = Vec::new(&env);
        approvers.push_back(signer);
        env.storage()
            .persistent()
            .set(&DataKey::ApprovalsRelease(id), &approvers);
        write_escrow(&env, id, &e);
    }
    ...
    pub fn approve_release(env: Env, id: u128, signer: Address) {
        ...
        signer.require_auth();
        let mut approvers = env
            .storage()
            .persistent()
            .get::<_, Vec<Address>>(&DataKey::ApprovalsRelease(id))
            .unwrap_or_else(|| Vec::new(&env));
        push_unique(&mut approvers, &signer);
        env.storage()
            .persistent()
            .set(&DataKey::ApprovalsRelease(id), &approvers);
        if approvers.len() >= e.release_threshold {
    ...
    fn execute_pending_release(env: Env, id: u128) {
        let mut e = read_escrow(&env, id);
        let prop: ReleaseProposal = env
            .storage()
            .persistent()
            .get(&DataKey::PendingRelease(id))
            .unwrap();
        ...
        if fee_total > 0 {
            client.transfer(&env.current_contract_address(), &fee_collector, &fee_total);
        }
        e.balance -= total;
        if e.balance == 0 {
            e.closed = true;
        ...
        env.storage()
            .persistent()
            .remove(&DataKey::PendingRelease(id));
        env.storage()
            .persistent()
            .remove(&DataKey::ApprovalsRelease(id));
        write_escrow(&env, id, &e);
    }

    pub fn propose_refund(env: Env, id: u128, signer: Address, dists: Vec<(Address, i128)>) {
        // dists target payers
        let e = read_escrow(&env, id);
        if e.closed {
            panic!("closed");
        ...
        if !is_member(&e.refund_signers, &signer) {
            panic!("not-refund-signer");
        }
        signer.require_auth();
        // validate recipients are payers
        for (p, _) in dists.iter() {
            if !is_member(&e.payers, &p) {
                panic!("bad-payer");
            }
        }
        let total = sum_amounts(&dists);
        ...
        env.storage()
            .persistent()
            .set(&DataKey::ApprovalsRefund(id), &approvers);
    }
    ...
    pub fn approve_refund(env: Env, id: u128, signer: Address) {
        ...
        let mut approvers = env
            .storage()
            .persistent()
            .get::<_, Vec<Address>>(&DataKey::ApprovalsRefund(id))
            .unwrap_or_else(|| Vec::new(&env));
        push_unique(&mut approvers, &signer);
        env.storage()
            .persistent()
            .set(&DataKey::ApprovalsRefund(id), &approvers);
        if approvers.len() >= e.refund_threshold {
            ...
            env.storage()
                .persistent()
                .remove(&DataKey::ApprovalsRefund(id));
            write_escrow(&env, id, &e);
        }
    }
    ...
    pub fn auto_release(env: Env, id: u128) {
        ...
        match e.auto_release_ts {
            Some(t) => {
                if now(&env) < t {
                    panic!("too-early");
                }
            }
            None => panic!("no-auto-release"),
        }
        ...
        let mut dists: Vec<(Address, i128)> = Vec::new(&env);
        ...
        for i in 0..e.payees.len() {
            ...
            dists.push_back((e.payees.get(i).unwrap(), amt));
        }
        drop(e);
        ...
        for _ in 0..e2.release_threshold {
            approvers.push_back(env.current_contract_address());
        }
        env.storage()
            .persistent()
            .set(&DataKey::ApprovalsRelease(id), &approvers);
    ...
    pub fn arbiter_release(env: Env, id: u128, signer: Address, dists: Vec<(Address, i128)>) {
        let e = read_escrow(&env, id);
        if e.closed {
            panic!("closed");
        }
        ...
        if !is_member(&e.arbiters, &signer) {
            panic!("not-arbiter");
        }
        signer.require_auth();
        if !ensure_payees_valid(&e.payees, &dists) {
        ...
        let mut approvers = env
            .storage()
            .persistent()
            .get::<_, Vec<Address>>(&DataKey::ApprovalsArbiter(id))
            .unwrap_or_else(|| Vec::new(&env));
        push_unique(&mut approvers, &signer);
        env.storage()
            .persistent()
            .set(&DataKey::ApprovalsArbiter(id), &approvers);
        if approvers.len() >= e.arbiter_threshold {
            // execute like normal release
            Self::execute_pending_release(env.clone(), id);
            // clear arbiter approvals and undispute if closed
            env.storage()
                .persistent()
                .remove(&DataKey::ApprovalsArbiter(id));
            let mut e2 = read_escrow(&env, id);
            if e2.balance == 0 {
                e2.disputed = false;
                e2.closed = true;
    ...
    pub fn arbiter_refund(env: Env, id: u128, signer: Address, dists: Vec<(Address, i128)>) {
        let mut e = read_escrow(&env, id);
        if e.closed {
            panic!("closed");
        }
    ...
    pub fn emergency_release(env: Env, id: u128, signer: Address, dists: Vec<(Address, i128)>) {
        let eadmins = read_vec_addr(&env, &DataKey::EmergencyAdmins);
        let ethresh = read_u32(&env, &DataKey::EmergencyThreshold);
        if !is_member(&eadmins, &signer) {
            panic!("not-emergency");
        }
        signer.require_auth();
        let mut e = read_escrow(&env, id);
        ...
        let mut approvers = env
            .storage()
            .persistent()
            .get::<_, Vec<Address>>(&DataKey::ApprovalsEmergency(id))
            .unwrap_or_else(|| Vec::new(&env));
        push_unique(&mut approvers, &signer);
        env.storage()
            .persistent()
            .set(&DataKey::ApprovalsEmergency(id), &approvers);
        if approvers.len() >= ethresh {
            Self::execute_pending_release(env.clone(), id);
            env.storage()
                .persistent()
                .remove(&DataKey::ApprovalsEmergency(id));
        }
    }

    // Views
    pub fn get_escrow(env: Env, id: u128) -> Escrow {
        read_escrow(&env, id)
    }
    pub fn get_fee_params(env: Env) -> (u32, Address) {
        (
            read_u32(&env, &DataKey::FeeBps),
            read_addr(&env, &DataKey::FeeCollector),
        )
    }
}
...
mod test {
    ...
    fn deploy_contract(e: &Env) -> (Address, super::MarketXEscrowClient) {
        let id = e.register(MarketXEscrow, ());
    ...
    fn test_full_flow() {
        ...
        let mut eadmins = Vec::new(&e);
        eadmins.push_back(emergency1.clone());
        eadmins.push_back(emergency2.clone());
        ...
        token_admin_client.mint(&payer1, &1_000);
        token_admin_client.mint(&payer2, &1_000);

        // create escrow
        let mut payers = Vec::new(&e);
        payers.push_back(payer1.clone());
        payers.push_back(payer2.clone());
        let mut payees = Vec::new(&e);
        payees.push_back(payee1.clone());
        payees.push_back(payee2.clone());
        let mut rels = Vec::new(&e);
        rels.push_back(payer1.clone());
        rels.push_back(payer2.clone());
        let refs = rels.clone();
        let mut arbs = Vec::new(&e);
        ...
        client.create_escrow(
            ...
            &EscrowInit {
                ...
                payers: payers.clone(),
                payees: payees.clone(),
                release_signers: rels.clone(),
                release_threshold: 2,
                refund_signers: refs.clone(),
                refund_threshold: 1,
                arbiters: arbs.clone(),
                arbiter_threshold: 2,
                auto_release_ts: Some(2000),
                expiry_ts: 3000,
            },
        );
        ...
        let mut dists = Vec::new(&e);
        dists.push_back((payee1.clone(), 300));
        dists.push_back((payee2.clone(), 100));
        ...
        let mut arb_d = Vec::new(&e);
        arb_d.push_back((payee1.clone(), 250));
        arb_d.push_back((payee2.clone(), 250));
        let arb1 = arbs.get(0).unwrap();
        let arb2 = arbs.get(1).unwrap();
        client.arbiter_release(&1u128, &arb1, &arb_d);
        client.arbiter_release(&1u128, &arb2, &arb_d);

        // After fee 2.5% each -> 244 and 244, fees 12 -> total fees 21
        assert_eq!(token_client.balance(&payee1), 293 + 244);
        assert_eq!(token_client.balance(&payee2), 98 + 244);
        assert_eq!(token_client.balance(&fee_collector), 9 + 12);

        // Remaining balance 100
        assert_eq!(client.get_escrow(&1u128).balance, 100);

        // Emergency release last 100 to payee1 by emergency admins
        let mut last = Vec::new(&e);
        last.push_back((payee1.clone(), 100));
        client.emergency_release(&1u128, &emergency1, &last);
        client.emergency_release(&1u128, &emergency2, &last);

        // Fee 2 -> net 98
        assert_eq!(token_client.balance(&payee1), 293 + 244 + 98);
        assert_eq!(token_client.balance(&fee_collector), 9 + 12 + 2);
    ...
    fn test_timeout_and_auto_release() {
        ...
        let (_contract_id, client) = deploy_contract(&e);
        let mut eadmins = Vec::new(&e);
        eadmins.push_back(Address::generate(&e));
        ...
        client.create_escrow(
            ...
            &EscrowInit {
                ...
                arbiters: arbs.clone(),
                arbiter_threshold: 1,
                auto_release_ts: None,
                expiry_ts: 1200,
            },
        );
        token_admin_client.mint(&payer, &600); // mint more for second escrow
