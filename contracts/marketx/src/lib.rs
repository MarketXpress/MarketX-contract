...
/// Step 1: Propose a new WASM hash code upgrade.
    pub fn propose_upgrade(env: Env, admin: Address, new_wasm_hash: BytesN<32>) {
        ...
        // 2. Calculate the execution availability milestone
        ...
        // 3. Persist the proposal to instance storage
        env.storage().instance().set(&DataKey::UpgradeProposal, &proposal);

        // 4. Emit event for off-chain indexers and user transparency
        env.events().publish(
    ...
    /// Step 2: Execute the proposed upgrade after the timelock expires.
    pub fn execute_upgrade(env: Env, admin: Address) {
        admin.require_auth();
        let current_admin: Address = env.storage().instance().get(&DataKey::Admin).unwrap();
        assert_eq!(admin, current_admin, "Not authorized");

        // 1. Retrieve the pending proposal
        ...
        // 2. Enforce the timelock restriction
        let current_ledger = env.ledger().sequence();
        assert!(
            current_ledger >= proposal.eta_ledger,
            "Timelock restriction active: current ledger {} < ETA {}",
            current_ledger,
            proposal.eta_ledger
        );

        // 3. Remove the proposal to prevent re-execution replay
        env.storage().instance().remove(&DataKey::UpgradeProposal);

        // 4. Perform the dynamic code migration
    ...
    /// Escape Hatch: Cancel a pending or compromised upgrade proposal immediately.
    pub fn cancel_upgrade(env: Env, admin: Address) {
        ...
        assert!(
            env.storage().instance().has(&DataKey::UpgradeProposal),
...
impl Contract {
    ...
    fn update_dispute_voting(env: &Env, escrow_id: u64) -> Result<(), ContractError> {
        let config: ArbitersConfig = env
            .storage()
            .persistent()
            .get(&DataKey::ArbitersConfig(escrow_id))
